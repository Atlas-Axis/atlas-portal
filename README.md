...
- `notion-ids-dev.ts` - Empty Notion identifiers for development and manual QA environments (empty strings/arrays)
...
2. **Development/QA**: Uses `notion-ids-dev.ts` (empty IDs) when `NODE_ENV !== 'production'` OR `USE_DEV_NOTION_IDS === 'true'`
   - Empty strings prevent accidental access to production data during local development and manual QA
...
- Development and manual QA environments use empty IDs to prevent accidental production data access
- Production uses real Notion IDs
...
- `NODE_ENV` - Either 'development' (for `npm run dev`) or 'production' (for `npm run build`). Controls which Notion IDs are loaded (production uses real IDs, non-production uses empty dev IDs)
- `USE_DEV_NOTION_IDS` - When set to `'true'`, forces the use of empty dev Notion IDs regardless of `NODE_ENV` (useful for manual QA in any environment)
