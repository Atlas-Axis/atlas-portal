...
During Notion imports (`convertNotionPagesToDatabaseFormat`):

1. Pages are converted from Notion format to Supabase format
2. For databases supporting internal nesting, mappings are loaded
3. `applyNestingOverrides()` processes each mapping:
...
4. Modified pages are saved to Supabase with correct relationships
...
1. **Identify the Problem**: Notice a document appearing under the wrong parent after import
...
5. **Re-import**: Trigger a Notion import for that database (you can use the `--local-cache` flag to make it much faster)
...
- **Integration**: `app/server/services/notion/convert-notion-pages-to-supabase-format.ts`
...
- The override logic runs **every time** a Notion import occurs for affected databases
