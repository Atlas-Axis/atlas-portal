...
- **scripts/filter-blue-json-inactive-docs.ts** — Filters `.debug-data/blue.json` to exclude all `inactive: 1` documents (including their subdocuments) and writes `.debug-data/atlas-json-generated/blue-without-inactive.json`.
  - Examples:
    - ```bash
...
- **scripts/atlas-json/filter-inactive-generic.ts** — Filters a Blue-style JSON input to exclude all `inactive: 1` entries (and their subtrees) and writes a filtered output file.
  - Examples:
    - ```bash
