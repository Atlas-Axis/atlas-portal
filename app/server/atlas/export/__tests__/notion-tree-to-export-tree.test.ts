...
vi.mock('../../formatters/atlas-rich-text-formatter', () => ({
  atlasDatabasePageToMarkdown: vi.fn().mockImplementation(
    ...
    (node: NotionAtlasTreeNode, uuidMappings: UuidMappings = mockUUIDMappings) => `# ${node.plain_text_name ?? ''}`,
  ),
}));
...
describe('notionTreeNodeToExportTreeNode', () => {
  ...
  it('maps base fields using stored values (atlas_document_number and plain_text_name) by default', () => {
    // By default, uses stored values from Supabase (from standardized Notion fields)
    // instead of dynamically generated values
    const node = makeNode({
    ...
    expect(result.content).toContain('Stored Name');
  });

  it('maps base fields using dynamic values (generatedDocID and generatedDocName) when useDynamicValues is true', () => {
    // When useDynamicValues is true (migration mode), uses dynamically generated values
    const node = makeNode({
      atlas_database_name: ATLAS_DATABASES.ARTICLES,
      atlas_document_type: 'Article',
      atlas_document_number: 'A.2', // Not used when useDynamicValues is true
      plain_text_name: 'Stored Name', // Not used when useDynamicValues is true
      generatedDocID: 'GEN.1',
      generatedDocName: 'Generated Name',
    });

    const result = notionTreeNodeToExportTreeNode(node, uuidMappings, { useDynamicValues: true });
    expect(result.type).toBe('Article');
  ...
  it('propagates useDynamicValues option to child nodes', () => {
    ...
    // Test with useDynamicValues: false (default)
    const resultStored = notionTreeNodeToExportTreeNode(root, uuidMappings) as ExportAtlasTreeScopesDocument;
    expect(resultStored.name).toBe('Stored Root');
    expect(resultStored.articles[0].name).toBe('Stored Child');

    // Test with useDynamicValues: true
    const resultDynamic = notionTreeNodeToExportTreeNode(root, uuidMappings, {
  ...
  it('preserves child order', () => {
    // Uses plain_text_name for name (stored value from Supabase)
